...
func (d *CircuitDetectorService) refreshLimits(ctx context.Context) {
	...
	// Build instrument keys ("EXCHANGE:SYMBOL") for EQUITY + MCX only.
	instruments := d.loadEquityMCXInstruments()
	if len(instruments) == 0 {
		log.Println("circuit detector: no EQUITY/MCX instruments found")
		return
	}
...
// loadEquityMCXInstruments fetches active instruments that belong to EQUITY or MCX-FUT segments.
// This explicitly filters by segment to avoid picking up NFO-FUT instruments that use exchange=NSE.
func (d *CircuitDetectorService) loadEquityMCXInstruments() []models.Instrument {
	if d.db == nil {
		return nil
	}
	...
	err := d.db.
		...
		Where("segment IN ?", []string{
			models.InstrumentSegmentEquity,
			models.InstrumentSegmentMCXFut,
		}).
